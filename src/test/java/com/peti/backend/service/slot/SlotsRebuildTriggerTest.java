...
class SlotsRebuildTriggerTest {
  ...
  @Mock
  private SlotCrudService slotCrudService;

  @InjectMocks
  private SlotsRebuildTrigger trigger;
  ...
  void rebuild_iteratesAllDays() {
    ...
    verify(slotCrudService, times(3))
  ...
  void rebuild_persistsGeneratedDocs() {
    ...
    ArgumentCaptor<List<ElasticSlotDocument>> docsCaptor =
        ArgumentCaptor.forClass(List.class);
    verify(slotCrudService).replaceSlotsByCaretakerAndDate(
