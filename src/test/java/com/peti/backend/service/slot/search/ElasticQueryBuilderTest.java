...
class ElasticQueryBuilderTest {
  ...
  void fullQuery() {
    ...
    // date From + To filters + cityId + minRating + capacity = 5 must clauses
    assertThat(query.bool().must()).hasSize(5);
  }
  ...
  void minimalQuery() {
    ...
    assertThat(query).isNotNull();
    assertThat(query.isBool()).isTrue();
    // date to+from filters + capacity(min 1) = 3 must clauses
    assertThat(query.bool().must()).hasSize(3);
  }
