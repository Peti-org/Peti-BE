 ...
 * Pet-preference matching is done in Java after retrieval because caretakerPreferences is stored as an opaque
 * (non-indexed) object.
 */
@Component
public class ElasticQueryBuilder {
  ...
  /**
   * Overlap logic: slot.fromDateTime <= requestEnd AND slot.toDateTime >= requestStart. Uses fromDateTime/toDateTime
   * fields as stored in the index.
   */
  private void addDateTimeFilters(List<Query> must, ElasticSlotSearchRequest req) {
    String requestStart = req.startDateTime().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME);
    String requestEnd = req.endDateTime().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME);
