...
public class PriceCalculationService {
  ...
  public PriceCalculationResult calculatePrice(ElasticSlotDocument slot, List<PetInfo> pets) {
    ...
    long durationMinutes = calculateDurationTime(slot);
  ...
  private long calculateDurationTime(ElasticSlotDocument slot) {
    if (slot.getFromDateTime() == null || slot.getToDateTime() == null) {
      return 0;
    }
    return Duration.between(slot.getFromDateTime(), slot.getToDateTime()).toMinutes();
  }
}
